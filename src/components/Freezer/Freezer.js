...
class Freezer extends Component {
  ...
  componentDidMount() {
    ...
    setInterval(() => {
      const randomTemperature = -Math.floor(Math.random() * 10);
  ...
  render() {
    const flavors = Object.keys(this.state.flavors).map(flavorName => (
      <FreezerFlavor key={flavorName} onClickRestock={() => this.handleClickRestock(flavorName)} flavorName={flavorName} scoops={this.state.flavors[flavorName]} />
    ));

    return (
      <Panel title={`Freezer (deg ${this.state.temperature || 0}C)`}>
